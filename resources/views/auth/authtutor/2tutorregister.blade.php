@section('content')
    <style>
        ...
        .register-photo .form-container .image-holder .test {
            /* margin-top: 25%; */
            padding-top: 50%;
        }
    </style>
    <div class="register-photo">
        <div class="form-container">
            <div class="image-holder">
                <div class=" test text-white px-3 py-2  mx-md-4  ">
                    <h4 class="mb-4">Welcome to Tutor Matching</h4>
                    <p class="small mb-0">Welcome to the tutor matching tutor register page.</p>
                </div>
            </div>
            <form method="POST" action="{{ route('tutor.register') }}">
                @csrf
                <h2 class="text-center"><strong>สร้างบัญชี.</strong></h2>
                <div class="form-group">
                    <input id="name" class="form-control @error('tutor_name') is-invalid @enderror"
                        type="text" name="tutor_name" value="{{ old('name') }}" required autocomplete="name" autofocus
                        placeholder="Name">
                    @error('name')
                ...
                <div class="mt-2 ">
                    <a href="{{ route('tutor.login') }}" class="already">คุณมีบัญชีอยู่แล้ว? เข้าสู่ระบบที่นี่.</a>
                </div>
            </form>
